const centsToReal = (val) => (typeof val === 'number' ? val / 100 : 0);
...
const buildStatsHtml = (stats) => {
    ...
    for (const k of [
        'total_pedidos', 'total_cancelados', 'total_aprovados',
        'taxa_cancelamento_percentual', 'receita_bruta_total', 'ticket_medio'
    ]) {
    ...
    for (const prod of produtos) {
        const receita = typeof prod.receita_gerada === 'number'
            ? prod.receita_gerada.toFixed(2)
            : esc(prod.receita_gerada);
...
for (const item of $input.all()) {
    ...
    let stats = {
        periodo: { inicio: null, fim: null, total_dias: 0 },
        kpis: {
            total_pedidos: 0,
            total_cancelados: 0,
            total_aprovados: 0,
    ...
    if (stats.periodo.inicio && stats.periodo.fim) {
        const diff = Math.abs(stats.periodo.fim - stats.periodo.inicio);

        stats.periodo.total_dias = Math.ceil(diff / (1000 * 60 * 60 * 24));
        stats.periodo.inicio = stats.periodo.inicio.toISOString().split('T')[0];
        stats.periodo.fim = stats.periodo.fim.toISOString().split('T')[0];
    }

    stats.kpis.taxa_cancelamento_percentual =
        ((stats.kpis.total_cancelados / stats.kpis.total_pedidos) * 100).toFixed(2) + '%';
    ...
    for (const p of listaProdutos) {
        p.receita_gerada = Number(p.receita_gerada.toFixed(2));
    }

    stats.kpis.receita_bruta_total = Number(stats.kpis.receita_bruta_total.toFixed(2));
